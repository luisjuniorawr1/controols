...
export default function ImageToolRunner({ tool, locale }: { tool: Tool; locale: Locale }) {
  const t=copy[locale]; const [files,setFiles]=useState<File[]>([]); const [option,setOption]=useState(''); const [base64,setBase64]=useState(''); const [preview,setPreview]=useState(''); const [output,setOutput]=useState(''); const [download,setDownload]=useState<{url:string;name:string}|null>(null); const [busy,setBusy]=useState(false);
  const needsSecond=tool.slug==='add-image-watermark'; const base64Input=tool.slug==='base64-to-image';
  const hint=useMemo(()=>({
    'resize-image':'1200,800','resize-image-by-percent':'50','crop-image':'x,y,width,height','adjust-image-brightness':'120','adjust-image-contrast':'120','adjust-image-saturation':'120','blur-image':'6','pixelate-image':'12','add-text-watermark':'CONTROOLS|40|40|36','add-image-watermark':'opacity 0.45','image-color-picker':'100,100','change-image-dpi':'300','compress-jpg':'75','compress-webp':'75'
  ...
  return <section className="runner asset-runner image-runner"><div className="asset-controls">{base64Input?<label><span>{t.input}</span><textarea value={base64} onChange={e=>setBase64(e.target.value)} placeholder="data:image/png;base64,…"/></label>:<label className="drop-zone"><input type="file" accept="image/*,.svg,.bmp" multiple={needsSecond} onChange={e=>setFiles(Array.from(e.target.files||[]))}/><strong>Select {needsSecond?'2 images':'image'}</strong><span>{files.map(f=>`${f.name} · ${(f.size/1024).toFixed(1)} KB`).join('\n')||'Your image stays in this browser'}</span></label>} {!base64Input&&![ 'view-image-metadata','image-to-base64','image-dimensions','remove-image-metadata','make-square-image','create-favicon'].includes(tool.slug)&&<label><span>Settings</span><textarea className="small-option" value={option} onChange={e=>setOption(e.target.value)} placeholder={hint}/></label>}<button className="primary" onClick={process} disabled={busy}>{busy?'…':t.run}</button></div><div className="asset-result">{preview?<img src={preview} alt="Image preview"/>:<div className="preview-empty">◫</div>}<textarea readOnly value={output} placeholder={t.output}/>{download&&<a className="download-button" href={download.url} download={download.name}>Download image</a>}</div></section>
}
