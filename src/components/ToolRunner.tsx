...
export default function ToolRunner({ tool, locale }: { tool: Tool; locale: Locale }) {
  ...
  if (pineImageExtraSlugs.has(tool.slug)) {
    const auto = !tool.slug.startsWith('bulk-') && tool.slug !== 'take-screenshot';
    return <AutoRunEnhancer enabled={auto}><StructuredPineImageToolRunner tool={tool} locale={locale}/></AutoRunEnhancer>;
  }
