...
old_types = "type Case002Screen = 'warning' | 'weak' | 'strong' | 'reuse' | 'code' | 'key' | 'ending';\n"
new_types = old_types + "type Case003Screen = 'warning' | 'clues' | 'address' | 'path' | 'clone' | 'map' | 'ending';\n"
if "type Case003Screen" not in text:
    text = text.replace(old_types, new_types, 1)

text = text.replace("<small>{index === 1 ? 'NOVO · ' : ''}CASO {game.number}</small>", "<small>{index === kidsGames.length - 1 ? 'NOVO · ' : ''}CASO {game.number}</small>")

marker = '\nexport default function KidsStoryPrototype() {'
if 'function Case003Game' not in text:
    case003 = r'''
...
'''
    ...
    text = text.replace(marker, case003 + marker, 1)

old_game = "  const game = gameId === 'case-001' ? firstKidsStory : secondKidsStory;\n  const assets = gameId === 'case-001' ? case001Assets : case002Assets;"
new_game = "  const game = gameId === 'case-001' ? firstKidsStory : gameId === 'case-002' ? secondKidsStory : thirdKidsStory;\n  const assets = gameId === 'case-001' ? case001Assets : gameId === 'case-002' ? case002Assets : case003Assets;"
if new_game not in text:
    if old_game not in text:
        raise SystemExit('game/assets selector not found')
    text = text.replace(old_game, new_game, 1)

old_render = "      {mode === 'playing' && gameId === 'case-002' && <Case002Game onExit={exitGame} />}"
new_render = old_render + "\n      {mode === 'playing' && gameId === 'case-003' && <Case003Game onExit={exitGame} />}"
if "gameId === 'case-003'" not in text:
    if old_render not in text:
        raise SystemExit('Case002 render line not found')
    text = text.replace(old_render, new_render, 1)
component.write_text(text)

qa = Path('tests/qa-smoke.spec.ts')
q = qa.read_text()
q = q.replace("number: '001' | '002'", "number: '001' | '002' | '003'")
q = q.replace("await expect(page.locator(`[data-screen=\"loading-case-00${number === '001' ? '1' : '2'}\"]`)).toBeVisible();", "await expect(page.locator(`[data-screen=\"loading-case-${number}\"]`)).toBeVisible();")
q = q.replace("const firstScreen = number === '001' ? 'case001-intro' : 'case002-warning';", "const firstScreen = number === '001' ? 'case001-intro' : number === '002' ? 'case002-warning' : 'case003-warning';")
q = q.replace("test('library shows two games and no character-selection step'", "test('library shows three games and no character-selection step'")
old_library = "  await expect(page.getByRole('button', { name: /caso 002/i })).toBeVisible();\n  await expect(page.getByText(/escolha seu amigo/i)).toHaveCount(0);"
new_library = "  await expect(page.getByRole('button', { name: /caso 002/i })).toBeVisible();\n  await expect(page.getByRole('button', { name: /caso 003/i })).toBeVisible();\n  await expect(page.getByText(/escolha seu amigo/i)).toHaveCount(0);"
if new_library not in q:
    q = q.replace(old_library, new_library, 1)
...
'''
phone_marker = "\ntest('game library also fits a phone viewport without scrolling'"
if "Case 003 complete flow" not in q:
    if phone_marker not in q:
        raise SystemExit('phone test marker not found')
    q = q.replace(phone_marker, case003_test + phone_marker, 1)

old_phone = "  await expect(page.getByRole('button', { name: /caso 002/i })).toBeVisible();\n  await expectSingleViewport(page);"
new_phone = "  await expect(page.getByRole('button', { name: /caso 002/i })).toBeVisible();\n  await expect(page.getByRole('button', { name: /caso 003/i })).toBeVisible();\n  await expectSingleViewport(page);"
if new_phone not in q:
    q = q.replace(old_phone, new_phone, 1)
qa.write_text(q)
