...
export default function sitemap(): MetadataRoute.Sitemap {
  ...
  const blogPages=locales.flatMap(locale=>blogPosts.map(post=>({url:`${base}${blogPath(post,locale)}`,lastModified:new Date(`${safeContentDate(post.updatedAt)}T12:00:00-03:00`),changeFrequency:'monthly' as const,priority:.8})));
