...
export default async function ToolPage({params}:{params:Promise<{locale:string;slug:string}>}){
  const {locale,slug}=await params;if(!isLocale(locale))notFound();const tool=getTool(slug);if(!tool)notFound();const cat=getCategory(tool.category)!;const t=copy[locale];const related=tools.filter(x=>x.category===tool.category&&x.slug!==slug).slice(0,6);const live=isToolLive(tool),title=toolTitle(tool,locale);
  return <><Header locale={locale}/><main className="inner"><div className="breadcrumbs"><Link href={`/${locale}/`}>Controols</Link><span>/</span><Link href={`/${locale}/category/${cat.id}/`}>{cat.labels[locale]}</Link><span>/</span><span>{title}</span></div><section className="tool-hero"><div><div className="eyebrow"><span className={`status-dot ${live?'live':''}`}/>{live?t.ready:t.building}</div><h1>{title}</h1><p>{t.browserNote}</p></div><div className="tool-category-badge"><span>{cat.icon}</span>{cat.labels[locale]}</div></section><ToolRunner tool={tool} locale={locale}/><section className="section compact"><div className="section-title"><h2>{cat.labels[locale]}</h2><Link href={`/${locale}/category/${cat.id}/`}>{t.allTools} →</Link></div><div className="related-grid">{related.map(x=><Link href={`/${locale}/tools/${x.slug}/`} key={x.slug}>{toolTitle(x,locale)}<span>↗</span></Link>)}</div></section></main></>;
}
